...
/** Search community Brawl lists (via analytics/dev proxy → Moxfield or Archidekt). */
export async function searchBrawl100Decks(commanderName: string): Promise<MoxfieldSearchResult> {
  const { searchBrawl100DecksProxied } = await import('@/services/brawl/fetchCommunityBrawlPopularity');
  const response = await searchBrawl100DecksProxied(commanderName);
  ...
  return { status: response.status || 403, numDecks: response.numDecks, cards: response.cards };
}
